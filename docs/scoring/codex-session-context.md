# Codex Session Context (REUSABLE CONTEXT block)

This file holds one thing: the exact text pasted fresh at the start of a Codex agentic-workflow
eval session, unedited. Paste the fenced block below verbatim. Do not summarize or partially paste
it. The METADATA-questionnaire-first opening inside it applies when this block is pasted fresh at
the start of a new session, not mid-session when a WF-xxx file is already being filled from pasted
evidence.

**V3.1 (2026-07-31) supersedes the 2026-07-27 version** stored here previously. What changed: the
whole block is now domain-agnostic (dropped the SEO/marketing-specific examples like "Search
Console, GA4, Clarity" in favor of generic "dashboards, sheets, docs, tickets, repos, chats,
databases," with an explicit "don't assume a domain, take it from the PROMPT"); WHO IS WRITING
gained a new paragraph on not letting the prompt's own "you" (addressed to the drafter) leak into
the answers; the Voice section gained two rules that were not present at all before, a fully
elaborated "no 'you' anywhere in a box" ban and a "plain prose, no symbols or markdown" ban (no
arrows, no slash-as-shorthand, no bullets, no bold, no headers inside commentary text), each with a
matching new bullet in the Before-you-send self-check; SCORING STANCE gained two new named
principles, RAISE THE BAR ("good enough is a 4, not a 6") and MATCH THE TONE TO THE NUMBER (a 5
must read like a 5, not 80% praise with one "but," and the accuracy box is named as the worst
offender for this); and the rating calibration for a 4 is now explicit that it's "a normal, common
score, NOT a failing grade," not just "mixed."
...
For the standing rules that apply on top of this block, the model codename mapping, and the actual
fillable form skeleton, see [feather-form-scratchpad.md](feather-form-scratchpad.md). For the
current, non-superseded scoring rules (flaw-hunting, scoring gates, how Overall gets derived), see
[harsh-evaluation-protocol.md](harsh-evaluation-protocol.md).
