...
C's core computation is fully correct, in line with every surviving run, and its tickets trace individual queries back to specific source chunks more thoroughly than either of the other two. It also opened and repaired its own finished sheet before handing the work back, catching a real display problem before it shipped, the clearest case of a run checking itself in this comparison. It isn't flawless: its closing summary includes a chunk of raw mathematical notation that will not display as intended outside a source view, and confirming its one layout fix took three separate narrow rereads instead of one. Those are real but comparatively minor drags, and they are why it leads the group rather than sweeping it cleanly.
