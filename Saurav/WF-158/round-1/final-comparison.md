...
Master template for scoring four Codex model runs against the same WF-xxx prompt, then giving one final cross-model ranking. Fill one full block per model, then the Final comparison block once at the end. Copy this file into the relevant WF-xxx folder while scoring, then transcribe into the actual OpenAI feedback form.

## Source: REUSABLE CONTEXT

This is the original context block pasted at the start of a Codex agentic-workflow eval session. It is the source the standing rules below are distilled from. This version (2026-07-27) supersedes the earlier draft: it natively specifies all 8 boxes (Task accuracy, ignoring speed is item 2), splits OUTPUT out as its own fourth paste item alongside METADATA/PROMPT/LOGS, and adds four large new sections not present in the earlier draft: WHO IS WRITING (trainer-voice framing), Source of truth, CHECK THE SOURCE YOURSELF FIRST, SCORING STANCE (with a "Harshness done RIGHT" guardrail against inventing flaws from things you can't see), BEFORE YOU SCORE, and a "Before you send" self-check. Where it disagrees with the standing rules below, the standing rules win, since those reflect direct corrections confirmed after this text was first written: the rating ceiling is a hard 6 in practice (this source text says 7 should be "essentially never" but doesn't forbid it outright). The METADATA-questionnaire-first opening applies when this block is pasted fresh at the start of a new session, not mid-session when a WF-xxx file is already being filled from pasted evidence.
